...
public class RegistrationService {
    ...
    public void registerNewClient(String identification, String name, String lastName,
                                  ...
                                  String accountType) throws DuplicateUserException {
        ...
        createAccountForClient(newClient, accountType);

        bankService.saveAllData();
        System.out.println("RegistrationService: Cliente " + username + " registrado con cuenta " + accountType);
    }

    public void addAccountToExistingClient(String identification, String accountType) throws UserNotFoundException {
        User user = bankService.findUserByIdentification(identification);
        if (user == null || !(user instanceof Client)) {
            throw new UserNotFoundException("No se encontró un cliente con la cédula: " + identification);
        }

        createAccountForClient((Client) user, accountType);
        bankService.saveAllData();
    }

    private void createAccountForClient(Client client, String accountType) {
        ...
        switch (accountType) {
            case "Corriente":
                ...
                newAccount = new CheckingAccount(newAccountNumber, 0.0, client, 500000.0);
                break;

            case "Empresarial":

                newAccount = new BusinessAccount(newAccountNumber, 0.0, client, 0.0, 0.0);
                break;

            default:

                newAccount = new SavingsAccount(newAccountNumber, 0.0, client, 0.02);
                break;
        }
    ...
    public void deleteEmployee(Employee employee) throws Exception {
        if (employee == null) throw new Exception("Nulo");
        if (employee.getUsername().equalsIgnoreCase("admin")) throw new Exception("No se puede eliminar al admin");
